...
function TransactionDetails({ transaction }: { transaction: TransactionType }) {
  ...
  if (!transaction) {
    return (
      <div className="p-10 text-center text-red-600 font-bold">
        ⚠️ No transaction data provided
      </div>
    );
  }

  // Temporarily commenting out destructuring since it crashes when fields are missing
  // const {
  //   amount,
  //   status: transactionStatus,
  //   createdAt,
  ...
  return (
    <section className="mx-auto max-h-screen overflow-scroll py-10 flex w-full max-w-[800px] flex-col items-center gap-10">
      <h2 className="font-dm_sans text-xl font-semibold text-siteHeadingDark">
        🚧 Transaction Details Page (Debug Mode)
      </h2>
      <pre className="bg-gray-100 p-4 rounded-lg max-w-full overflow-x-scroll text-sm">
